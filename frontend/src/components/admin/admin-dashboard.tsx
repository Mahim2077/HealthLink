  ...
  return (
    <main className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:px-8 sm:py-14 lg:px-10" id="main-content">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <section className="rounded-[1.75rem] bg-indigo-950 p-6 text-white shadow-xl shadow-indigo-950/10 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.15em] text-indigo-300">Access level</p><h2 className="mt-3 text-2xl font-bold">{admin.is_super_admin ? "Super administrator" : "Administrator"}</h2><p className="mt-3 text-sm leading-6 text-indigo-100/75">Your trusted operational account is active. Protected actions are recorded for accountable review.</p><span className="mt-6 inline-flex rounded-full bg-emerald-300/15 px-3 py-1.5 text-xs font-bold text-emerald-200">ACTIVE</span></section>
        <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.15em] text-indigo-700">Admin identity</p><h2 className="mt-3 text-xl font-bold text-slate-950">{admin.first_name} {admin.last_name}</h2><dl className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-slate-50 p-4"><dt className="text-xs font-bold text-slate-500">Email</dt><dd className="mt-1 break-words text-sm font-semibold text-slate-900">{admin.email}</dd></div><div className="rounded-xl bg-slate-50 p-4"><dt className="text-xs font-bold text-slate-500">Account type</dt><dd className="mt-1 text-sm font-semibold text-slate-900">Trusted operational account</dd></div></dl><div className="mt-6 grid gap-3 sm:grid-cols-2"><Link className="inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-700 px-4 text-center text-sm font-bold text-white" href="/admin/professional-registrations">Review professionals</Link><Link className="inline-flex min-h-11 items-center justify-center rounded-xl border border-indigo-200 bg-indigo-50 px-4 text-center text-sm font-bold text-indigo-800" href="/admin/facilities">Manage facilities</Link></div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2"><Link className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-center text-sm font-bold text-slate-700" href="/admin/citizen-identities">Citizen identity support</Link></div></section>
      </div>
    </main>
  );
}
