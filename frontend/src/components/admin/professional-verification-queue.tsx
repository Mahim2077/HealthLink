...
function ProfessionalVerificationQueueContent() {
  ...
  const visibleRows = rows?.filter(row => `${row.first_name} ${row.last_name} ${row.role_name} ${row.facility_name_submitted}`.toLowerCase().includes(query.trim().toLowerCase())) ?? null;
  ...
  return <main className="hl-page" id="main-content">
    ...
    <div className="mt-7 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><label className="text-sm font-bold text-slate-700">Search this queue<input className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal" placeholder="Name, role, or submitted facility" value={query} onChange={(event) => setQuery(event.target.value)} /></label><p aria-live="polite" className="text-sm font-semibold text-slate-600">{visibleRows?.length ?? 0} shown</p></div>
    <div aria-label="Filter applications" className="mt-4 flex flex-wrap gap-2">{filters.map((item) => <button aria-pressed={filter === item.value} className={`min-h-11 rounded-xl px-4 text-sm font-bold ${filter === item.value ? "bg-indigo-700 text-white" : "border border-slate-300 bg-white text-slate-700"}`} key={item.label} onClick={() => setFilter(item.value)} type="button">{item.label}</button>)}</div>
